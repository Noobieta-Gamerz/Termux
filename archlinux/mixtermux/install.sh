...
DISTRO_PATH=$PREFIX/var/lib/proot-distro/installed-rootfs/\${DISTRO:-"archlinux"}

[ -f ~/.mixedtermuxrc ] && source ~/.mixedtermuxrc
! command -v proot-distro > /dev/null && pkg install -y proot-distro
! [ -d \$DISTRO_PATH ] && ! [ -z "\$(ls -A \$DISTRO_PATH)" ] && proot-distro install \${DISTRO:-"archlinux"}
...
ARGS=("proot-distro login \${DISTRO:-archlinux}")
